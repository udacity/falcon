...
;(function() {
    ...
    describe('Your code', function() {
        ...
        it('should have a template literal `greeting`', function() {
            assert.equal('TemplateLiteral', matches[0].init.type, "The `greeting` string should be changed to a template literal");
        });
    });
    ...
    describe('Your template literal', function() {
        ...
        it('should match the original `greeting` string', function() {
            assert.equal('Hello, my name is myName', matches[0].init.quasis[0].value.raw + matches[0].init.expressions[0].name, "The value of the `greeting` template literal doesn't match the original `greeting` string");
        });
    });
    }
